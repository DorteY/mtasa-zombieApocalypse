...
setRadarSettings = function()
    local radarSettings = {
        ["mapTexture"] = ":"..getResourceName(getThisResource()).."/Files/Images/Radar/Map.png",
        ["mapTextureSize"] = 3072,
    ...
    Fontss = {}
    Fontss.Roboto = "default-bold"
    Fontss.Icons = "default-bold"
...
function drawRadar()
    if isLoggedin() and getElementInterior(localPlayer) == 0 then
        ...
        dxDrawBorder(Radar.PosX, Radar.PosY, Radar.Width, Radar.Height, 3, tocolor(0, 0, 0, 150))
            ...
            for _, blip in ipairs(serverBlips) do
                ...
                if localPlayer ~= getElementAttachedTo(blip) and getElementInterior(localPlayer) == getElementInterior(blip) and getElementDimension(localPlayer) == getElementDimension(blip) then
                    ...
                    dxSetBlendMode("modulate_add")
                    dxDrawImage(blipX, blipY, 24, 24, "Files/Images/Radar/Blips/"..bid..".png", 0, 0, 0, tocolor(bR, bG, bB, 255), false)
                    dxSetBlendMode("blend")
                end
            end      
            ...
            dxDrawImage((Radar.PosX + (Radar.Width / 2)) - 10, (Radar.PosY + (Radar.Height / 2)) - 10, 20, 20, "Files/Images/Radar/Blips/Player.png", camZ - rz, 0, 0)
