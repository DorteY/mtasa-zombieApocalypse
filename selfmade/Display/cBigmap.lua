...
function drawMap()
	if isLoggedin() and getElementInterior(localPlayer) == 0 then
		if not toggle then
			dxDrawImage(0,0,0,0,":"..getResourceName(getThisResource()).."/Files/Images/Radar/Map.png",0,0,0,0,false)
		else
			checkMovement()
			...
			dxDrawImage(x,y,hSize,vSize,":"..getResourceName(getThisResource()).."/Files/Images/Radar/Map.png",0,0,0,mapDrawColor,false)
...
function drawLocalPlayerArrow()
	...
	dxDrawImage(mapX-8,mapY-8,16,16,"Files/Images/Radar/Blips/Player.png",(-r)%360,0,0,normalColor,false)
end
...
function setPlayerMapImage(image,tLX,tLY,lRX,lRY)
	...
	elseif not image then
		...
		mapFile                           = "Files/Images/Radar/Map.png"
